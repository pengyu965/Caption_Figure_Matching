...
class Model:
    def __init__(self, batch_size=50, lr = 0.0001, keep_prob=0.4, class_num=2, is_training = True):
        ...
        self.image_input = tf.placeholder(tf.float32, [self.batch_size, None, None, 3], name='input_image') 
            ...
            with tf.variable_scope('image_conv1', reuse=tf.AUTO_REUSE):
                self.layer['image_conv1_layer'] = tf.nn.relu(
                    tf.nn.conv2d(
                        ...
                        filter = tf.get_variable('c1w', [5,5, 3, 128], trainable=self.is_training), 
                        strides=[1,2,2,1],
    ...
    def fc(self):
        flatten_image_feature = tf.reshape(self.layer['image_pool2_layer'], (self.batch_size, 65536))
        ...
        with tf.variable_scope('fc1',reuse=tf.AUTO_REUSE):
            self.layer['fc1_layer'] = tf.keras.layers.Dense(units=512, activation=tf.nn.relu)(feature)
        ...
        for ep in range(self.epoch):
            ...
            for idi in range(idx):
                ...
                for i in range(len(batch_fn)):
                    batch_image_data.append(batch_fn[i][0][0])
                    batch_sentence_data.append(batch_fn[i][0][1])
                ...
                _, loss_val, acc_val= sess.run(
                    ...
                    feed_dict={self.model.image_input: batch_image_data, self.model.sentence_input:batch_sentence_data, self.model.label: batch_label}
                )
    ...
    def validation(self, sess, writer, global_step):
        ...
        for val_idi in range(val_idx):
            ...
            for i in range(len(val_batch_fn)):
                val_batch_image_data.append(val_batch_fn[i][0][0])
                val_batch_sentence_data.append(val_batch_fn[i][0][1])
                val_batch_label.append(val_batch_fn[i][0][2])

            val_loss_val, val_acc_val= sess.run(
                    (self.loss, self.accuracy),
                    feed_dict={self.model.image_input: val_batch_image_data, self.model.sentence_input:val_batch_sentence_data, self.model.label: val_batch_label}
                )
...
def data_processing(dic_list, word2vec_model, category):
    ...
    for dic in dic_list:
        ...
        sent_matrix = np.expand_dims(np.array(sent_matrix), axis = -1).tolist()
        
        img = scipy.misc.imresize(scipy.misc.imread(img_path),(64,64)).tolist()
